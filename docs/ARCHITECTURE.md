...
**Sleep/wake lifecycle hooks.** `MxModelLoader` implements vLLM's optional `BaseModelLoader.on_sleep(level)` / `on_wake_up(tags)` / `on_weights_reloaded()` hooks so a sleeping replica (`--enable-sleep-mode`) stops advertising itself as a P2P source before `sleep()` unmaps its GPU memory — otherwise a peer reading from it could segfault it.
- `on_sleep` calls `unpublish_metadata` (gates discovery) and `nixl_manager.shutdown()` (aborts in-flight requests from peers already connected).
- `on_wake_up` re-publishes and recreates the NIXL agent only for level 1, where cumem has already restored weight content by the time it fires; level 2 defers to `on_weights_reloaded`, since discarded weights aren't valid again until `reload_weights()` repopulates them.
