...
class TestSleepWakeReloadHooks:
    ...
    it's valid again. Only sleep level 1 is supported for wake-up.
    """
    ...
    def test_on_sleep_skips_unpublish_for_non_level_1(self):
        """Level 2 (discard) support is deferred, so on_sleep leaves
        source-serving running rather than stopping it for a sleep it
        cannot resume from."""
        loader = _make_loader()
        loader._ctx = _make_load_context()
        with patch("modelexpress.engines.vllm.loader.unpublish_metadata") as mock_unpub:
    ...
    def test_on_wake_up_skips_for_non_level_1_sleep(self, last_sleep_level):
        ...
        valid when wake_up() returns; MxModelLoader doesn't support it, so
        on_wake_up must not trust the caller and must refuse to republish.
        None covers wake_up() called with no prior on_sleep() at all
        (unexpected from vLLM) -- that must not be treated as an implicit
        level-1 sleep either."""
        loader = _make_loader()
        loader._ctx = _make_load_context()
